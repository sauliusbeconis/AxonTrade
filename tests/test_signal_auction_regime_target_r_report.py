...
def test_reports_target_r_selected_after_auction_guard() -> None:
    bars = []
    signal_rows = []
    regime_rows = []
    for index, trade_date in enumerate(("2026-06-10", "2026-06-11", "2026-06-12"), start=1):
        bars.extend(
            [
                ...
                _bar(trade_date, 1, high=102.25, low=99.5, close=102),
            ],
        )
    ...
    assert list(report_rows[0].keys()) == SIGNAL_AUCTION_REGIME_TARGET_R_REPORT_HEADER
    assert [row["sample"] for row in report_rows] == ["train", "holdout"]
    assert report_rows[0]["target_r_multiple"] == "2"
    assert report_rows[0]["evaluated_trades"] == 2
    assert report_rows[1]["target_r_multiple"] == "2"
    assert report_rows[1]["evaluated_trades"] == 1
    assert report_rows[1]["target_hits"] == 1
    assert report_rows[1]["auction_skipped_trades"] == 1
    assert report_rows[1]["auction_skipped_losses"] == 1
    assert report_rows[1]["auction_skipped_net_usd"] == "-200"


def test_requires_train_target_rows_to_meet_minimum() -> None:
    with pytest.raises(SignalAuctionRegimeTargetReportError, match="minimum_train_trades"):
        report_signal_auction_regime_target_r(
            bars=[],
