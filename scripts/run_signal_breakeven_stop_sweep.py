#!/usr/bin/env python3
"""Run breakeven-stop sweeps over logged Sierra signal rows."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sweep target R and breakeven stop trigger R for logged signals.",
    )
    ...
    parser.add_argument("output", help="Path to write breakeven-stop sweep CSV rows.")
    ...
    try:
        ...
        experiment_rows = run_signal_breakeven_stop_sweep(
            normalized_rows,
            signal_rows,
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=SIGNAL_BREAKEVEN_STOP_SWEEP_HEADER,
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    print(
        f"wrote {len(experiment_rows)} breakeven-stop sweep rows to {output_path}; "
        f"best={best_summary}",
    )
    return 0
