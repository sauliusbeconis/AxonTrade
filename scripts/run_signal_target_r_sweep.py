#!/usr/bin/env python3
"""Run target R-multiple sweeps over logged Sierra signal rows."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sweep target R multiples for already logged candidate signals.",
    )
    ...
    parser.add_argument("output", help="Path to write target R sweep CSV rows.")
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=SIGNAL_TARGET_R_SWEEP_HEADER,
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    print(
        f"wrote {len(experiment_rows)} target R sweep rows to {output_path}; "
        f"best={best_summary}",
    )
    return 0
