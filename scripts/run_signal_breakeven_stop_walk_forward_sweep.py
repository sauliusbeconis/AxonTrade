#!/usr/bin/env python3
"""Run rolling walk-forward breakeven-stop sweeps over logged Sierra signals."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run rolling breakeven-stop walk-forward selection for logged signals.",
    )
    ...
    parser.add_argument("output", help="Path to write breakeven-stop walk-forward CSV rows.")
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=SIGNAL_BREAKEVEN_STOP_WALK_FORWARD_HEADER,
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    print(
        f"wrote {len(split_rows)} breakeven-stop walk-forward rows to {output_path}; "
