#!/usr/bin/env python3
"""Run chronological auction-regime filter sweeps."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run chronological auction-regime filter selection.",
    )
    parser.add_argument("regime_diagnostics", help="Path to auction-regime diagnostic CSV rows.")
    parser.add_argument("output", help="Path to write auction-regime filter sweep CSV rows.")
    ...
    print(
        f"wrote {len(split_rows)} auction-regime filter sweep rows to {output_path}; "
