#!/usr/bin/env python3
"""Run rolling walk-forward target R sweeps over logged Sierra signal rows."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run rolling target R walk-forward selection for logged signals.",
    )
    ...
    parser.add_argument("output", help="Path to write target R walk-forward CSV rows.")
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=SIGNAL_TARGET_R_WALK_FORWARD_SWEEP_HEADER,
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    print(
        f"wrote {len(split_rows)} target R walk-forward rows to {output_path}; "
