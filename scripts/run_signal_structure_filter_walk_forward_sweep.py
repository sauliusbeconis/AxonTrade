#!/usr/bin/env python3
"""Run rolling walk-forward structure-aware entry filter sweeps."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run rolling structure-filter walk-forward selection.",
    )
    parser.add_argument("diagnostics", help="Path to signal quality diagnostic CSV rows.")
    parser.add_argument("output", help="Path to write structure filter walk-forward CSV rows.")
    parser.add_argument(
        "--train-date-count",
        type=int,
        default=8,
    ...
    _write_csv(Path(args.output), SIGNAL_STRUCTURE_FILTER_WALK_FORWARD_HEADER, split_rows)
    ...
    print(
        f"wrote {len(split_rows)} structure filter walk-forward rows to {args.output}; "
