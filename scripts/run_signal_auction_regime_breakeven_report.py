#!/usr/bin/env python3
"""Report selected auction-regime guards stacked with breakeven exits."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Report selected auction-regime rules stacked with breakeven exits.",
    )
    ...
    parser.add_argument("--target-r-multiples", default="1,1.25,1.5,2,2.5,3")
    parser.add_argument("--breakeven-trigger-r-multiples", default="0.5,0.75,1,1.25,1.5")
    ...
    _write_csv(output_path, SIGNAL_AUCTION_REGIME_BREAKEVEN_REPORT_HEADER, report_rows)
    ...
    print(
        f"wrote {len(report_rows)} auction-regime breakeven rows to {output_path}; "
