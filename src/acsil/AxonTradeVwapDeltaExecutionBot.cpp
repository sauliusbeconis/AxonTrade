...
const char* kRequiredMnqTopRunnerLiveConfirmationText = "MNQ_TOP_RUNNER_LIVE";
...
const char* StrategyIdForTradeMode(const std::string& trade_mode)
{
    ...
    if (trade_mode == "mnq_top_runner_live")
        return kMnqTopRunnerLiveStrategyId;
    return kStrategyId;
}
...
void RunMnqTopRunnerLiveStudy(SCStudyInterfaceRef sc)
{
    SCInputRef CsvLogPath = sc.Input[0];
    SCInputRef ArmExecution = sc.Input[1];
    SCInputRef SendOrdersToTradeService = sc.Input[2];
    ...
    SCInputRef SetupEndTime = sc.Input[11];
    SCInputRef FlattenTime = sc.Input[12];
    SCInputRef Quantity = sc.Input[13];
    SCInputRef MaxPositionQuantity = sc.Input[14];
    SCInputRef DailyLossLimitUsd = sc.Input[15];
    SCInputRef DailyProfitLockUsd = sc.Input[16];
    SCInputRef LookbackBars = sc.Input[17];
    SCInputRef BufferPoints = sc.Input[18];
    SCInputRef DeltaThreshold = sc.Input[19];
    SCInputRef DirectionalCloseLocationThreshold = sc.Input[20];
    SCInputRef TargetPoints = sc.Input[21];
    SCInputRef StopPoints = sc.Input[22];
    SCInputRef MinimumSignalSpacingSeconds = sc.Input[23];
    SCInputRef DrawStatusBannerInput = sc.Input[24];
    SCInputRef StatusBannerVerticalPosition = sc.Input[25];
    SCInputRef StatusBannerFontSize = sc.Input[26];
    SCInputRef AcceptedSetupAlertSound = sc.Input[27];
    SCInputRef DrawTradeMarkersAndLevels = sc.Input[28];
    SCInputRef TradeLevelForwardBars = sc.Input[29];

    const int status_slot = 6;

    if (sc.SetDefaults)
    {
        sc.GraphName = "AxonTrade MNQ Top Runner Live Bot";
        sc.StudyDescription =
            "Guarded live-capable MNQ normal-profitability lookback-breakout runner bot using the lower-DD 120/70 variant.";
        sc.AutoLoop = 0;
        sc.GraphRegion = 0;
        sc.UpdateAlways = 1;
        ...
        CsvLogPath.Name = "CSV Log Path";
        CsvLogPath.SetString("C:\\SierraChart\\Data\\AxonTrade_MnqTopRunnerLiveBot.csv");
        ...
        DailyLossLimitUsd.Name = "Daily Loss Lock USD";
        DailyLossLimitUsd.SetFloat(300.0f);
        ...
        StatusBannerVerticalPosition.Name = "Status Banner Vertical Position";
        StatusBannerVerticalPosition.SetInt(76);
        StatusBannerVerticalPosition.SetIntLimits(5, 98);
    ...
    const bool route_to_trade_service = SendOrdersToTradeService.GetYesNo() != 0;
    const std::string trade_mode = "mnq_top_runner_live";
    ...
    int& last_processed_bar_index = sc.GetPersistentInt(91);
    int& full_recalculation_reset_done = sc.GetPersistentInt(92);
    int& processing_initialized = sc.GetPersistentInt(93);
    ...
    const bool confirmation_ok =
        ToStdString(ConfirmationText.GetString()) == kRequiredMnqTopRunnerLiveConfirmationText;
    const bool symbol_ok = StartsWith(symbol, required_symbol_prefix);
    const bool simulation_mode_ok =
        RequireTradeSimulationModeOffForLive.GetYesNo() == 0 || sc.GlobalTradeSimulationIsOn == 0;
    const bool account_ok = !allowed_trade_account.empty() && trade_account == allowed_trade_account;
    ...
    if (DrawStatusBannerInput.GetYesNo() != 0)
    {
        ...
        if (ArmExecution.GetYesNo() == 0)
            headline = "AXON MNQ TOP RUNNER LIVE: STANDBY - NOT ARMED";
        else if (csv_log_path.empty())
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - CSV PATH BLANK";
            background_color = RGB(128, 32, 24);
        }
        else if (!route_to_trade_service)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - ROUTING OFF";
            background_color = RGB(128, 32, 24);
        }
        else if (!confirmation_ok)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - CONFIRMATION TEXT";
            background_color = RGB(128, 32, 24);
        }
        else if (!simulation_mode_ok)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - SIERRA SIM MODE IS ON";
            background_color = RGB(128, 32, 24);
        }
        else if (allowed_trade_account.empty())
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - ALLOWED ACCOUNT BLANK";
            background_color = RGB(128, 32, 24);
        }
        else if (!account_ok)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - ACCOUNT MISMATCH";
            background_color = RGB(128, 32, 24);
        }
        else if (!symbol_ok)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: BLOCKED - SYMBOL PREFIX";
            background_color = RGB(128, 32, 24);
        }
        else if (current_chart_downloading_historical_data)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: WAIT - HISTORICAL DOWNLOAD";
            background_color = RGB(96, 72, 0);
        }
        else if (daily_lock_active)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: LOCKED - DAILY RISK";
            background_color = RGB(128, 32, 24);
        }
        else if (immediate_position_data.PositionQuantity != 0.0 || immediate_position_data.WorkingOrdersExist != 0)
        {
            headline = "AXON MNQ TOP RUNNER LIVE: ARMED - MANAGING POSITION/ORDERS";
            background_color = RGB(0, 92, 72);
        }
        else
        {
            headline = "AXON MNQ TOP RUNNER LIVE: ARMED - READY";
            background_color = RGB(0, 96, 32);
        }

        std::ostringstream gate_line;
        gate_line << "Gates: arm=" << YesNoStatus(ArmExecution.GetYesNo() != 0)
                  << " route=" << (route_to_trade_service ? "LIVE" : "OFF")
                  << " sim=" << (sc.GlobalTradeSimulationIsOn != 0 ? "ON" : "OFF")
                  << " simGate=" << YesNoStatus(simulation_mode_ok)
                  << " confirm=" << YesNoStatus(confirmation_ok)
                  << " acct=" << YesNoStatus(account_ok)
    ...
    for (int bar_index = start_bar_index; bar_index <= latest_closed_bar_index; ++bar_index)
    {
        ...
        const bool current_simulation_mode_ok =
            RequireTradeSimulationModeOffForLive.GetYesNo() == 0 || sc.GlobalTradeSimulationIsOn == 0;
        const bool current_account_ok =
            !allowed_trade_account.empty() && trade_account == allowed_trade_account;
        const bool order_functions_allowed =
            ArmExecution.GetYesNo() != 0
            && current_route_to_trade_service
            && current_confirmation_ok
            && current_simulation_mode_ok
            && current_account_ok
            && StartsWith(symbol, required_symbol_prefix)
        ...
        else if (!current_route_to_trade_service)
        {
            can_submit = false;
            rejection_reason = "routing_off";
            rejection_notes = "Send Orders To Trade Service must be Yes for MNQ top-runner live routing";
        }
        else if (!current_confirmation_ok)
        {
            can_submit = false;
            rejection_reason = "confirmation_text_missing";
            rejection_notes = std::string("Confirmation Text must be ")
                + kRequiredMnqTopRunnerLiveConfirmationText;
        }
        else if (!current_simulation_mode_ok)
        {
            can_submit = false;
            rejection_reason = "trade_simulation_mode_must_be_off";
            rejection_notes = "Trade >> Trade Simulation Mode On must be off for MNQ top-runner live routing";
        }
        else if (allowed_trade_account.empty())
        {
            can_submit = false;
        ...
        else if (sc.IsFullRecalculation != 0)
        {
            can_submit = false;
            rejection_reason = "full_recalculation_order_block";
            rejection_notes = "MNQ top-runner live bot does not submit orders during full recalculation";
        }
        else if (selected_quantity <= 0)
        {
            can_submit = false;
        ...
        entry_notes << candidate.notes
                    ...
                    << "; target1_offset_points=" << FormatNumber(TargetPoints.GetFloat())
                    << "; minimum_signal_spacing_seconds=" << MinimumSignalSpacingSeconds.GetInt()
                    << "; daily_loss_lock_usd=" << FormatNumber(DailyLossLimitUsd.GetFloat())
                    << "; route_to_trade_service=true";
...
SCSFExport scsf_AxonTradeMnqTopRunnerLiveBot(SCStudyInterfaceRef sc)
{
    RunMnqTopRunnerLiveStudy(sc);
}
