"""Auction-regime guard plus breakeven-stop reporting for logged signals."""
...
class SignalAuctionRegimeBreakevenReportError(ValueError):
    """Raised when the auction-regime/breakeven report cannot be computed."""


def report_signal_auction_regime_breakeven(
    *,
    ...
    """Stack selected auction-regime rules with train-selected breakeven exits."""
    ...
    if minimum_train_trades <= 0:
        raise SignalAuctionRegimeBreakevenReportError("minimum_train_trades must be positive")
    ...
    for train_selection, holdout_selection in _selected_train_holdout_pairs(selection_rows):
        ...
        best_train = _select_best_train_exit_row(
        ...
        matching_holdout = _find_matching_exit_row(holdout_sweep, best_train)
...
def _select_best_train_exit_row(
    ...
    *,
    ...
    if not eligible_rows:
        raise SignalAuctionRegimeBreakevenReportError(
            f"No train breakeven experiments met minimum_train_trades={minimum_train_trades}",
        )
...
def _combined_row(
    ...
    *,
    ...
    net_usd = _to_float(exit_row["net_usd"], "net_usd")
    total_candidate_net = net_usd + auction_skipped["net_usd"]
    experiment_id = (
        f"signal_auction_regime_breakeven:strategy={exit_row['strategy_id']}:"
        ...
        f"exit_direction={exit_row['direction_filter']}:"
        f"target_r={exit_row['target_r_multiple']}:"
        f"breakeven_trigger_r={exit_row['breakeven_trigger_r']}"
    )
    return {
        "schema_version": 1,
        "experiment_id": experiment_id,
        "strategy_id": exit_row["strategy_id"],
        ...
        "exit_direction_filter": exit_row["direction_filter"],
        "target_r_multiple": exit_row["target_r_multiple"],
        "breakeven_trigger_r": exit_row["breakeven_trigger_r"],
        ...
        "input_signal_rows": exit_row["input_signal_rows"],
        "input_candidates": exit_row["input_candidates"],
        "evaluated_trades": exit_row["evaluated_trades"],
        "target_hits": exit_row["target_hits"],
        "losses": exit_row["losses"],
        "breakeven_exits": exit_row["breakeven_exits"],
        "other_exits": exit_row["other_exits"],
        "win_rate": exit_row["win_rate"],
        "gross_usd": exit_row["gross_usd"],
        "net_usd": exit_row["net_usd"],
        "average_net_usd": exit_row["average_net_usd"],
        "total_candidate_net_usd": _format_number(total_candidate_net),
        "long_trades": exit_row["long_trades"],
        "short_trades": exit_row["short_trades"],
        "notes": (
            "selected auction-regime guard first, then train-selected target R "
            "and breakeven trigger on auction-eligible logged candidates"
        ),
    }
...
def _find_matching_exit_row(
    rows: list[dict[str, Any]],
    selected_row: dict[str, Any],
) -> dict[str, Any]:
    selected_key = _exit_selection_key(selected_row)
    for row in rows:
        if _exit_selection_key(row) == selected_key:
            return row
    raise SignalAuctionRegimeBreakevenReportError(
        "Missing matching holdout breakeven row for "
        f"exit_selection={selected_key}",
    )
...
def _tag_split_row(
    ...
    *,
    ...
    tagged = {
        ...
        "split_id": split_id,
        "sample": sample,
        "selected_on_train": str(_exit_selection_key(row) == _exit_selection_key(selected_row)).lower(),
        "trade_dates": ";".join(trade_dates),
    }
    tagged.update(
        {
            key: row[key]
            for key in SIGNAL_AUCTION_REGIME_BREAKEVEN_REPORT_HEADER
...
def _exit_selection_key(row: dict[str, Any]) -> tuple[str, str, str]:
    direction_filter = row.get("exit_direction_filter", row.get("direction_filter"))
    return (
        str(direction_filter),
...
def _parse_timestamp(value: str) -> datetime:
    ...
    for timestamp_format in _TIMESTAMP_FORMATS:
        try:
            ...
            continue
    raise SignalAuctionRegimeBreakevenReportError(f"Invalid timestamp: {value!r}")
...
def _normalize_positive_grid(values: Iterable[float], field_name: str) -> list[float]:
    ...
    if any(value <= 0 for value in grid):
        raise SignalAuctionRegimeBreakevenReportError(f"{field_name} values must be positive")
    return grid
