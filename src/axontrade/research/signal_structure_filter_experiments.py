"""Structure-aware entry filter experiments for logged signal diagnostics."""
...
SIGNAL_STRUCTURE_FILTER_SWEEP_HEADER = [
    ...
    "min_sweep_aggression_ratio",
    "min_confirmation_edge_close",
...
class SignalStructureFilterExperimentError(ValueError):
    """Raised when a structure filter experiment cannot be evaluated."""


def run_signal_structure_filter_sweep(
    ...
    *,
    ...
    max_bars_after_sweep_values: Iterable[float],
    min_sweep_aggression_ratios: Iterable[float],
    min_confirmation_edge_closes: Iterable[float],
    direction_filters: Iterable[str] = ("all",),
) -> list[dict[str, Any]]:
    """Sweep entry-known structure filters over signal diagnostics."""
    ...
    max_bars = _normalize_positive_grid(
        max_bars_after_sweep_values,
        "max_bars_after_sweep_values",
    )
    min_aggression_ratios = _normalize_positive_grid(
        min_sweep_aggression_ratios,
        "min_sweep_aggression_ratios",
    )
    min_edge_closes = _normalize_edge_close_grid(min_confirmation_edge_closes)
    ...
    for (
        ...
        filtered_rows = _filter_rows(
            ...
            min_sweep_aggression_ratio=min_aggression_ratio,
            min_confirmation_edge_close=min_edge_close,
        )
...
def run_signal_structure_filter_walk_forward_sweep(
    diagnostic_rows: Iterable[dict[str, Any]],
    *,
    ...
    """Run rolling selection of structure filters by trade date."""

    rows = list(diagnostic_rows)
    dates = _sorted_trade_dates(rows)
    if train_date_count <= 0:
        raise SignalStructureFilterExperimentError("train_date_count must be positive")
    if holdout_date_count <= 0:
        raise SignalStructureFilterExperimentError("holdout_date_count must be positive")
    if minimum_train_trades <= 0:
        raise SignalStructureFilterExperimentError("minimum_train_trades must be positive")
    if train_date_count + holdout_date_count > len(dates):
        raise SignalStructureFilterExperimentError(
    ...
    for window_index in range(max_start):
        ...
        train_sweep = run_signal_structure_filter_sweep(
            ...
            max_bars_after_sweep_values=max_bars,
            min_sweep_aggression_ratios=min_aggression_ratios,
            min_confirmation_edge_closes=min_edge_closes,
        ...
        holdout_sweep = run_signal_structure_filter_sweep(
        ...
        split_id = (
            f"structure_filter_walk_forward_window={window_index + 1}:"
...
def _filter_rows(
    ...
    *,
    ...
    max_bars_after_sweep: float,
    min_sweep_aggression_ratio: float,
    min_confirmation_edge_close: float,
) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if _to_float(row["sweep_abs_delta"], "sweep_abs_delta") > max_sweep_abs_delta:
            continue
        if _to_float(row["bars_after_sweep"], "bars_after_sweep") > max_bars_after_sweep:
            continue
        if (
            _to_float(row["sweep_aggression_ratio"], "sweep_aggression_ratio")
            < min_sweep_aggression_ratio
        ):
            continue
        if _confirmation_edge_close(row) < min_confirmation_edge_close:
            continue
...
def _experiment_row(
    ...
    *,
    ...
    experiment_id = (
        f"signal_structure_filter:strategy={strategy_id}:"
        ...
        f"max_bars_after_sweep={_format_number(max_bars_after_sweep)}:"
        f"min_sweep_aggression_ratio={_format_number(min_sweep_aggression_ratio)}:"
        f"min_confirmation_edge_close={_format_number(min_confirmation_edge_close)}"
    )
    ...
    return {
        ...
        "max_bars_after_sweep": _format_number(max_bars_after_sweep),
        "min_sweep_aggression_ratio": _format_number(min_sweep_aggression_ratio),
        "min_confirmation_edge_close": _format_number(min_confirmation_edge_close),
        ...
        "notes": (
            "structure filter sweep over original outcome diagnostics; "
            "uses only entry-known sweep timing, aggression, and confirmation fields"
        ),
    }
...
def _find_matching_selection_row(
    ...
    for row in rows:
        if _selection_key(row) == selected_key:
            return row
    raise SignalStructureFilterExperimentError(
        "Missing matching holdout structure filter row for "
...
def _tag_split_row(
    ...
    *,
    ...
    tagged.update(
        {
            key: row[key]
            for key in SIGNAL_STRUCTURE_FILTER_SWEEP_HEADER
...
def _selection_key(row: dict[str, Any]) -> tuple[str, ...]:
    return (
        ...
        str(row["max_bars_after_sweep"]),
        str(row["min_sweep_aggression_ratio"]),
        str(row["min_confirmation_edge_close"]),
    )


def _confirmation_edge_close(row: dict[str, Any]) -> float:
    close_location = _to_float(row["confirmation_close_location"], "confirmation_close_location")
...
def _parse_timestamp(value: str) -> datetime:
    ...
    for timestamp_format in _TIMESTAMP_FORMATS:
        try:
            ...
            continue
    raise SignalStructureFilterExperimentError(f"Invalid timestamp: {value!r}")
...
def _normalize_positive_grid(values: Iterable[float], field_name: str) -> list[float]:
    ...
    if not grid:
        raise SignalStructureFilterExperimentError(f"{field_name} must contain at least one value")
    if any(value <= 0 for value in grid):
        raise SignalStructureFilterExperimentError(f"{field_name} values must be positive")
    return grid
...
def _normalize_nonnegative_grid(values: Iterable[float], field_name: str) -> list[float]:
    ...
    if not grid:
        raise SignalStructureFilterExperimentError(f"{field_name} must contain at least one value")
    if any(value < 0 for value in grid):
        raise SignalStructureFilterExperimentError(f"{field_name} values must be nonnegative")
    return grid


def _normalize_edge_close_grid(values: Iterable[float]) -> list[float]:
    grid = [float(value) for value in values]
    if not grid:
        raise SignalStructureFilterExperimentError(
            "min_confirmation_edge_closes must contain at least one value",
        )
    if any(value < 0 or value > 1 for value in grid):
        raise SignalStructureFilterExperimentError(
            "min_confirmation_edge_closes values must be between 0 and 1",
        )
    return grid
