"""Context-aware entry filter experiments for logged signal diagnostics."""
...
SIGNAL_CONTEXT_FILTER_SWEEP_HEADER = [
    ...
    "max_target_distance_to_average_bar_range",
    "max_sweep_abs_delta_to_average_abs_delta",
...
class SignalContextFilterExperimentError(ValueError):
    """Raised when a context filter experiment cannot be evaluated."""


def run_signal_context_filter_sweep(
    ...
    *,
    ...
    max_target_distance_to_average_bar_ranges: Iterable[float],
    max_sweep_abs_delta_to_average_abs_deltas: Iterable[float],
    ...
    """Sweep entry-known context filters over signal context diagnostics."""
    ...
    max_target_ranges = _normalize_positive_grid(
        max_target_distance_to_average_bar_ranges,
        "max_target_distance_to_average_bar_ranges",
    )
    max_sweep_ratios = _normalize_positive_grid(
        max_sweep_abs_delta_to_average_abs_deltas,
        "max_sweep_abs_delta_to_average_abs_deltas",
    )
    ...
    for (
        ...
        filtered_rows = _filter_rows(
            ...
            max_target_distance_to_average_bar_range=max_target_range,
            max_sweep_abs_delta_to_average_abs_delta=max_sweep_ratio,
...
def run_signal_context_filter_walk_forward_sweep(
    context_rows: Iterable[dict[str, Any]],
    *,
    ...
    """Run rolling selection of context-aware entry filters by trade date."""

    rows = list(context_rows)
    dates = _sorted_trade_dates(rows)
    if train_date_count <= 0:
        raise SignalContextFilterExperimentError("train_date_count must be positive")
    if holdout_date_count <= 0:
        raise SignalContextFilterExperimentError("holdout_date_count must be positive")
    if minimum_train_trades <= 0:
        raise SignalContextFilterExperimentError("minimum_train_trades must be positive")
    if train_date_count + holdout_date_count > len(dates):
        raise SignalContextFilterExperimentError(
    ...
    for window_index in range(max_start):
        ...
        train_sweep = run_signal_context_filter_sweep(
            ...
            max_risk_to_average_bar_ranges=max_risk_ranges,
            max_target_distance_to_average_bar_ranges=max_target_ranges,
            max_sweep_abs_delta_to_average_abs_deltas=max_sweep_ratios,
            min_entry_volume_to_average_volumes=min_volume_ratios,
            min_entry_trades_to_average_trades=min_trade_ratios,
        ...
        holdout_sweep = run_signal_context_filter_sweep(
        ...
        split_id = (
            f"context_filter_walk_forward_window={window_index + 1}:"
...
def _filter_rows(
    ...
    *,
    ...
    max_target_distance_to_average_bar_range: float,
    max_sweep_abs_delta_to_average_abs_delta: float,
    ...
    for row in rows:
        ...
        if (
            _to_float(
                row["target_distance_to_average_bar_range"],
        ...
        if (
            _to_float(
                row["sweep_abs_delta_to_average_abs_delta"],
...
def _experiment_row(
    ...
    *,
    ...
    experiment_id = (
        f"signal_context_filter:strategy={strategy_id}:"
        ...
        f"max_target_avg_range={_format_number(max_target_distance_to_average_bar_range)}:"
        f"max_sweep_avg_delta={_format_number(max_sweep_abs_delta_to_average_abs_delta)}:"
    ...
    return {
        ...
        "max_target_distance_to_average_bar_range": _format_number(
            max_target_distance_to_average_bar_range,
        ),
        "max_sweep_abs_delta_to_average_abs_delta": _format_number(
            max_sweep_abs_delta_to_average_abs_delta,
        ),
        "min_entry_volume_to_average_volume": _format_number(
            min_entry_volume_to_average_volume,
        ),
        "min_entry_trades_to_average_trades": _format_number(
            min_entry_trades_to_average_trades,
        ),
        ...
        "notes": (
            "context filter sweep over original outcome diagnostics; "
            "uses only entry-known rolling volatility/activity fields"
        ),
    }
...
def _find_matching_selection_row(
    ...
    for row in rows:
        if _selection_key(row) == selected_key:
            return row
    raise SignalContextFilterExperimentError(
        "Missing matching holdout context filter row for "
...
def _tag_split_row(
    ...
    *,
    ...
    tagged.update(
        {
            key: row[key]
            for key in SIGNAL_CONTEXT_FILTER_SWEEP_HEADER
...
def _selection_key(row: dict[str, Any]) -> tuple[str, ...]:
    return (
        ...
        str(row["max_target_distance_to_average_bar_range"]),
        str(row["max_sweep_abs_delta_to_average_abs_delta"]),
...
def _parse_timestamp(value: str) -> datetime:
    ...
    for timestamp_format in _TIMESTAMP_FORMATS:
        try:
            ...
            continue
    raise SignalContextFilterExperimentError(f"Invalid timestamp: {value!r}")
...
def _normalize_positive_grid(values: Iterable[float], field_name: str) -> list[float]:
    ...
    if not grid:
        raise SignalContextFilterExperimentError(f"{field_name} must contain at least one value")
    if any(value <= 0 for value in grid):
        raise SignalContextFilterExperimentError(f"{field_name} values must be positive")
    return grid
...
def _normalize_nonnegative_grid(values: Iterable[float], field_name: str) -> list[float]:
    ...
    if not grid:
        raise SignalContextFilterExperimentError(f"{field_name} must contain at least one value")
    if any(value < 0 for value in grid):
        raise SignalContextFilterExperimentError(f"{field_name} values must be nonnegative")
    return grid
